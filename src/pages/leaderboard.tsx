...
import { DarkModeButton, Menu, MessageAlert, Footer } from '@Components/index';
  ...
  useEffect(() => {
    ...
    for (let item of leaderboard) {
      for (let wager of item.wagerData) {
        if (
          wager.mint.toBase58() === currency.mint.toBase58() &&
          wager.totalWager.toNumber() !== 0
        ) {
          allItems.push({
            wallet: item.wallet,
